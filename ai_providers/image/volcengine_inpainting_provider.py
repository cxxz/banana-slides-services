...
class VolcengineInpaintingProvider:
    ...
    def inpaint_image(
        ...
        try:
            ...
            service.set_scheme("https")  # Must use HTTPS for authentication
            
            # 使用SDK的img2img_inpainting方法
            logger.info("使用SDK img2img_inpainting发送请求（带正确签名）")

            try:
                # 使用SDK的img2img_inpainting方法
                response = service.img2img_inpainting(request_body)
                ...
                # 尝试从错误信息中提取JSON响应
                try:
                    # 可能是 bytes 对象被 str() 包装
                    if error_str.startswith("b'"):
                        # 从 b'...' 格式中提取内容
                        import ast
                        raw_bytes = ast.literal_eval(error_str)
                        response = json.loads(raw_bytes.decode('utf-8'))
                    else:
                        logger.error(f"无法解析错误响应: {error_str}")
                        return None
