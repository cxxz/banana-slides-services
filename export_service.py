...
import tempfile
import img2pdf
from prompts import get_clean_background_prompt
    ...
    def create_editable_pptx_from_mineru(
        ...
        Args:
            ...
            template_dir: Optional directory containing template PPTX files for styling
                         (title-slide.pptx and non-title-slide.pptx)

        Returns:
            PPTX file as bytes if output_file is None
        """
        from utils.pptx_builder import PPTXBuilder
        from utils.template_style_extractor import TemplateStyleExtractor
        ...
        # Extract styles from templates if provided
        style_config = None
        if template_dir and Path(template_dir).exists():
            try:
                extractor = TemplateStyleExtractor()
                style_config = extractor.extract_styles(template_dir)
                logger.info(f"Loaded template styles from: {template_dir}")
                logger.info(f"  Title font: {style_config.title_font.name}")
                logger.info(f"  Body font: {style_config.body_font.name}")
            except Exception as e:
                logger.warning(f"Failed to extract template styles: {e}")
                logger.info("Falling back to default styling")

        # Create PPTX builder with style config
