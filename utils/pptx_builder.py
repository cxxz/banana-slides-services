...
if TYPE_CHECKING:
    from utils.template_style_extractor import StyleConfig
...
class PPTXBuilder:
    ...
    def __init__(
        ...
        Args:
            ...
            style_config: Optional StyleConfig for template-based font and color styling
        """
    ...
    def set_style_config(self, style_config: 'StyleConfig'):
        """Set or update the style configuration"""
        self.style_config = style_config
    ...
    def add_text_element(
        ...
        is_title = text_level == 1 or text_level == 'title'

        # Apply template styling if available, otherwise just make titles bold
        if self.style_config:
            self._apply_font_style(paragraph.font, is_title)
        else:
            # Legacy behavior: just make title text bold
            if is_title:
    ...
    def _apply_font_style(self, font, is_title: bool):
        """
        Apply font style from StyleConfig to a font object.

        Args:
            font: The paragraph.font object to style
            is_title: Whether this is title/heading text
        """
        if not self.style_config:
            return

        font_style = self.style_config.title_font if is_title else self.style_config.body_font
                ...
                for col_idx, cell_text in enumerate(row_data):
                    if col_idx < cols:  # Safety check
                        ...
                        for paragraph in text_frame.paragraphs:
                            ...
                            if self.style_config:
                                # Use body font for table cells
                                self._apply_font_style(paragraph.font, is_title=False)
                                # Override bold for header row
                                if row_idx == 0:
                                    paragraph.font.bold = True
                            else:
                                # Legacy behavior: header row bold
                                if row_idx == 0:
                                    paragraph.font.bold = True
